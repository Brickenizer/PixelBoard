...
#include "PixelWifiServer.h"        // Your header if needed
#include <WiFi.h>
...
#include "pacman/pacman.h"     // For pacmanSetup
...
// Makes up to MAX_WIFI_ATTEMPTS tries, then falls back to AP mode
// instead of rebooting — so the device remains reachable for
// reconfiguration even when the target network is unavailable.
// -------------------------------------------------------------------
...
static void connectToWiFi() {
  ...
  if (!g_hasCredentials) {
    // No credentials saved => go straight to AP onboarding mode
    Serial.println("[WiFi] No credentials stored. Starting AP onboarding mode.");
    startAccessPoint();
    return;
  }
  ...
  while (WiFi.status() != WL_CONNECTED && attempts < MAX_WIFI_ATTEMPTS) {
    delay(2000);
    attempts++;
    Serial.printf("[WiFi] Attempt %d of %d — status: %d\n",
                  attempts, MAX_WIFI_ATTEMPTS, WiFi.status());
  }
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    Serial.printf("[WiFi] Failed to connect after %d attempts. "
                  "Falling back to AP mode (192.168.4.1).\n", MAX_WIFI_ATTEMPTS);
    WiFi.disconnect(true);
    delay(100);
    startAccessPoint();
  }
...
// -------------------------------------------------------------------
// Handler for /style.css - inlined CSS (no SPIFFS required)
// -------------------------------------------------------------------
...
body {
  ...
  @media screen and (max-width: 768px) {
    ...
    .controls-panel:not(.active) {
      display: none;  /* Only hide when explicitly not active */
    }
